...
let allDataFR = [cat0Data, cat1Data, cat2Data, cat3Data, cat4Data, cat5Data];
  ...
  for (let catData in allData) {
    ...
    if (catData != 0 && catData != nbCat) {
      for (let i = 0; i < nbProj[catData] - 1; i++) {
        for (let x in allData[catData][i].Logiciels)
          allLogiciels.push(allData[catData][i].Logiciels[x]);
      }
    }
  }
...
function fillProjectContent(n, m) {
  ...
  <h3>${allDataFR[n][m].Description}${
    allDataFR[n][m].Year !== undefined ? ", " + allDataFR[n][m].Year : ""
  }</h3>
  ...
  <div class="project-content-desc">
    <div class="desc">
      ${
        allDataFR[n][m].contentDesc !== undefined
          ? allDataFR[n][m].contentDesc
          : ""
      }
    </div>
